    ...
    <div class="container mt-5">
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id_factura'])) {
            ...
            if ($factura) {
                ...
                echo '<p>ID Factura: ' . htmlspecialchars($factura['id_factura']) . '</p>';
                echo '<p>ID Cliente: ' . htmlspecialchars($factura['id_cliente']) . '</p>';
                echo '<p>Fecha: ' . htmlspecialchars($factura['fecha']) . '</p>';
                echo '<p>Total: ' . htmlspecialchars($factura['total']) . '</p>';
                echo '<p>Impuestos: ' . htmlspecialchars($factura['impuestos']) . '</p>';
                echo '<p>Descuentos: ' . htmlspecialchars($factura['descuentos']) . '</p>';
                echo '<p>Total Final: ' . htmlspecialchars($factura['total_final']) . '</p>';
                echo '<p>Estado: ' . htmlspecialchars($factura['estado']) . '</p>';
        ...
        <form action="form_editar_vista.php" method="GET">
            <div class="form-group">
                <label for="id_factura" class="text-light font-weight-bold">ID de Factura:</label>
