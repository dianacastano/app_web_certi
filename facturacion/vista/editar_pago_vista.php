...
<body>
    <div class="container mt-5">
        <h2 class="text-center lead" style="background-color: blanchedalmond; font-size: xx-large"><strong>Selecciona ID</strong></h2>
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id_pago'])) {
            ...
            if ($pago) {
                ...
                echo '<div class="alert alert-success" role="alert">Factura encontrada:</div>';
                echo '<p>ID Pago: ' . htmlspecialchars($pago['id_pago']) . '</p>';
                echo '<p>ID Factura: ' . htmlspecialchars($pago['id_factura']) . '</p>';
                echo '<p>Fecha de Pago: ' . htmlspecialchars($pago['fecha_pago']) . '</p>';
                echo '<p>Importe: ' . htmlspecialchars($pago['importe']) . '</p>';
                echo '<p>Metodo de Pago: ' . htmlspecialchars($pago['metodo_pago']) . '</p>';

                // Puedes mostrar más datos según sea necesario
            } else {
                // Manejar el caso donde no se encontró el contacto
                echo '<div class="alert alert-danger" role="alert">No se encontró ninguna pago con el ID proporcionado.</div>';
            }
        }
        ?>

        <form action="form_pagos_vista.php" method="GET">
            <div class="form-group">
                <label for="id_pago" class="text-light font-weight-bold">ID de Pago:</label>
        ...
        <div class="text-center">
            <a href="../index_facturacion.php" class="btn btn-primary">
